package com.rixon.tsp.impl;

import com.rixon.tsp.Coordinate;
import com.rixon.tsp.Distance;

public class XYCoordinate implements Coordinate {

	private int x;
	private int y;
	
	public XYCoordinate(int x, int y) {
		super();
		this.x = x;
		this.y = y;
	}

	@Override
	public Distance getDistance(Coordinate coordinate) {
		double value = Math.sqrt(Math.pow(x-coordinate.getX(),2)+Math.pow(y-coordinate.getY(),2));
		return new DistanceInMeter(String.valueOf(value));
	}

	@Override
	public int getX() {
		return x;
	}

	@Override
	public int getY() {
		return y;
	}

	@Override
	public String toString() {
		return "XYCoordinate [x=" + x + ", y=" + y + "]";
	}

	@Override
	public int hashCode() {
		final int prime = 31;
		int result = 1;
		result = prime * result + x;
		result = prime * result + y;
		return result;
	}

	@Override
	public boolean equals(Object obj) {
		if (this == obj)
			return true;
		if (obj == null)
			return false;
		if (getClass() != obj.getClass())
			return false;
		XYCoordinate other = (XYCoordinate) obj;
		if (x != other.x)
			return false;
		if (y != other.y)
			return false;
		return true;
	}
	
	
}
